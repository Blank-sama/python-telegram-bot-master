...
from TeleGenic.utils.helpers import DefaultValue, DEFAULT_NONE
...
def pytest_runtestloop(session):
    session.add_marker(
        pytest.mark.filterwarnings('ignore::TeleGenic.utils.deprecate.TeleGenicDeprecationWarning')
    )
...
def bot(bot_info):
    ...
    return DictExtBot(bot_info['api_key'], private_key=PRIVATE_KEY)
...
def make_bot(bot_info, **kwargs):
    ...
    """
    return ExtBot(bot_info['api_key'], private_key=PRIVATE_KEY, **kwargs)
...
def make_message(text, **kwargs):
    """
    Testing utility factory to create a fake ``TeleGenic.Message`` with
    reasonable defaults for mimicking a real message.
    :param text: (str) message text
    :return: a (fake) ``TeleGenic.Message``
    """
    return Message(
        message_id=1,
...
def make_command_message(text, **kwargs):
    """
    Testing utility factory to create a message containing a single TeleGenic
    command.
    Mimics the TeleGenic API in that it identifies commands within the message
    ...
    :return: a (fake) ``TeleGenic.Message`` containing only the command
    """
...
def make_message_update(message, message_factory=make_message, edited=False, **kwargs):
    ...
    ``TeleGenic.Message`` or a string. In the latter case ``message_factory``
    is used to convert ``message`` to a ``TeleGenic.Message``.
    :param message: either a ``TeleGenic.Message`` or a string with the message text
    :param message_factory: function to convert the message text into a ``TeleGenic.Message``
    :param edited: whether the message should be stored as ``edited_message`` (vs. ``message``)
    :return: ``TeleGenic.Update`` with the given message
    """
...
def expect_bad_request(func, message, reason):
    """
    Wrapper for testing bot functions expected to result in an :class:`TeleGenic.error.BadRequest`.
...
def check_shortcut_signature(
    ...
    Args:
        shortcut: The shortcut, e.g. :meth:`TeleGenic.Message.reply_text`
        bot_method: The bot method, e.g. :meth:`TeleGenic.Bot.send_message`
...
def check_defaults_handling(
    ...
    def make_assertion(_, data, timeout=DEFAULT_NONE, df_value=DEFAULT_NONE):
        ...
        for arg in (dkw for dkw in kwargs_need_default if dkw != 'timeout'):
            # 'None' should not be passed along to TeleGenic
