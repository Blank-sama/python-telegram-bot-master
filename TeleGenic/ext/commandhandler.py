...
from TeleGenic.ext import BaseFilter, filters as filter_module
...
class CommandHandler(Handler[Update, CCT]):
    ...
    def __init__(
        ...
        filters: filter_module.BaseFilter = None,
        ...
        if filters:
            self.filters = filter_module.UpdateType.MESSAGES & filters
        else:
            self.filters = filter_module.UpdateType.MESSAGES
        ...
        if allow_edited is not None:
            ...
            if not allow_edited:
                self.filters &= ~filter_module.UpdateType.EDITED_MESSAGE
